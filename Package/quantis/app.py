...
app = Dash("Quantis", title="Quantis", assets_folder=str(ASSETS_PATH))
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.Dropdown(
                id="input_format", options=["Scavager", "MaxQuant"], value="Scavager",
        ...
        du.Upload(id="sample_input", filetypes=['tsv'], text="Upload Sample File", max_files=1),
        html.Details([
            html.Summary("What is sample file?", style={'font-size': '16pt'}),
            html.P(
                "Sample file is a tsv file, containing informtion about input files. "
        ...
        html.Details([
            ...
            html.Table([
                ...
                html.Tr([
                    ...
                    html.Td(dcc.Dropdown(id="threshold_calculation", options=["static", "semi-dynamic", "dynamic"], value="static", clearable=False)),
                ]),
    ...
    html.Div([
        ...
        html.H3("Differentiallly Expressed Proteins"),
...
def write_control_files(status: du.UploadStatus):
    return ";".join([file.name for file in status.uploaded_files])

@du.callback(
    Output("lastfiles_A", "value"),
    id="test_input"
)
def write_test_files(status: du.UploadStatus):
    return ";".join([file.name for file in status.uploaded_files])
...
def load_sample_file(status: du.UploadStatus):
    ...
    sdf = pd.read_csv(FILES_PATH / status.uploaded_files[0].name, sep='\t')
...
def run_quantis(
    ...
    control_files, test_files, imputation,
...
def save_proteins(_, data):
    ...
    return dcc.send_data_frame(df, "DE_proteins.csv")
...
def start_webview(app: Dash):
    webview.create_window(app.title, app.server, width=1200, height=800)  # type: ignore
    webview.start()
